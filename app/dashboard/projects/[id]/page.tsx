...
export default function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [nicstudyProps, setNicstudyProps] = useState<NicStudyProps>({
  ...
  console.log("isNicstudy", isNicstudy)

  const canRender = isNicstudy
    ? (nicstudyProps.title.trim().length > 0 && nicstudyProps.tips.length > 0)
    : localComps.length > 0 && localComps.every(comp => {
  ...
  return (
    <div className="space-y-6 pb-20 max-w-7xl overflow-hidden">
          ...
          <div className="flex items-center gap-2 shrink-0">
                      ...
                      if (isAsync) {
                        try {
                          for (let i = 0; i < renderCount; i++) {
                            ...
                            const payload: any = { projectId, compositionType: (project as any)?.compositionType };
                            if (isNicstudy) {
                              Object.assign(payload, nicstudyProps);
                            }
                        ...
                        for (let i = 0; i < renderCount; i++) {
                          const renderPayload: any = { projectId, compositionType: (project as any)?.compositionType };
                          if (isNicstudy) {
                            Object.assign(renderPayload, nicstudyProps);
                          }
                          await renderVideo(renderPayload);
            ...
            <Link href={`/dashboard/projects/${projectId}/renders`}>
              ...
              >
                ...
                <span className="hidden md:inline text-xs font-bold">{isNicstudy ? 'Carousel' : 'Videos'}</span>
              </Button>
            </Link>
      ...
      <div className="flex justify-center">
        <Card className="w-full max-w-[340px] border-border/60 bg-muted/20 overflow-hidden shadow-2xl shadow-primary/5">
          <div className="aspect-[9/16] relative flex flex-col items-center justify-center bg-black/90 group">
            {isNicstudy ? (
              <ImageIcon className="size-16 text-muted-foreground/20 animate-pulse" />
            ) : (
            ...
            <div className="mt-4 text-muted-foreground font-medium tracking-tight text-center px-4">
              {isNicstudy ? 'NicStudy Carousel Preview' : 'Portrait Preview Placeholder'}
            </div>
            <div className="absolute bottom-4 right-4 text-[10px] text-muted-foreground bg-black/40 px-2 py-1 rounded backdrop-blur-md border border-white/5 uppercase tracking-widest font-mono">
              {isNicstudy ? '1080x1910' : '1080x1920'}
            </div>
          </div>
      ...
      {/* 3. Horizontal Compositions Sequence / NicStudy Editor */}
      {isNicstudy ? (
